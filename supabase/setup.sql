-- ============================================================
-- MathBattle: Complete Setup Script
...
create policy "Admins can read own profile" on public.admins for select
to authenticated using (id = auth.uid());

drop policy if exists "Admins can update own profile" on public.admins;
create policy "Admins can update own profile" on public.admins for update
to authenticated using (id = auth.uid()) with check (id = auth.uid());

drop policy if exists "Authenticated users can create admin profile" on public.admins;
create policy "Authenticated users can create admin profile" on public.admins for insert
to authenticated with check (id = auth.uid());

drop policy if exists "Students can join without account" on public.students;
create policy "Students can join without account" on public.students for insert
to anon with check (true);

drop policy if exists "Admins can read students in own rooms" on public.students;
create policy "Admins can read students in own rooms" on public.students for select
to authenticated using (
  exists (
    select 1 from public.room_participants rp
    join public.rooms r on r.id = rp.room_id
    where rp.student_id = students.id and r.admin_id = auth.uid()
  )
);

drop policy if exists "Active room students are visible for leaderboard" on public.students;
create policy "Active room students are visible for leaderboard" on public.students for select
to anon using (
  exists (
    select 1 from public.room_participants rp
    join public.rooms r on r.id = rp.room_id
    where rp.student_id = students.id and r.status in ('waiting', 'live', 'ended')
  )
);

drop policy if exists "Admins manage own packages" on public.question_packages;
create policy "Admins manage own packages" on public.question_packages for all
...
create policy "Admins manage own questions" on public.questions for all
to authenticated
...
create policy "Admins manage own rooms" on public.rooms for all
to authenticated using (admin_id = auth.uid()) with check (admin_id = auth.uid());

drop policy if exists "Students can find open rooms by code" on public.rooms;
create policy "Students can find open rooms by code" on public.rooms for select
to anon using (status in ('waiting', 'live'));

drop policy if exists "Students can enter open rooms" on public.room_participants;
create policy "Students can enter open rooms" on public.room_participants for insert
to anon with check (
  exists (select 1 from public.rooms r where r.id = room_id and r.status in ('waiting', 'live'))
);

drop policy if exists "Active participants are visible in room" on public.room_participants;
create policy "Active participants are visible in room" on public.room_participants for select
to anon using (
  exists (select 1 from public.rooms r where r.id = room_id and r.status in ('waiting', 'live', 'ended'))
);

drop policy if exists "Admins can read own participants" on public.room_participants;
create policy "Admins can read own participants" on public.room_participants for select
to authenticated using (
  exists (select 1 from public.rooms r where r.id = room_id and r.admin_id = auth.uid())
);

drop policy if exists "Participants can submit answers while live" on public.student_answers;
create policy "Participants can submit answers while live" on public.student_answers for insert
to anon with check (
  exists (
...
create policy "Ended room answer review is visible" on public.student_answers for select
to anon using (
  exists (select 1 from public.rooms r where r.id = room_id and r.status = 'ended')
);

drop policy if exists "Admins can read own room answers" on public.student_answers;
create policy "Admins can read own room answers" on public.student_answers for select
to authenticated using (
  exists (select 1 from public.rooms r where r.id = room_id and r.admin_id = auth.uid())
);

drop policy if exists "Ended room results are visible" on public.battle_results;
create policy "Ended room results are visible" on public.battle_results for select
to anon using (
  exists (select 1 from public.rooms r where r.id = room_id and r.status = 'ended')
);

drop policy if exists "Admins can read own results" on public.battle_results;
create policy "Admins can read own results" on public.battle_results for select
to authenticated using (
  exists (select 1 from public.rooms r where r.id = room_id and r.admin_id = auth.uid())
);

drop policy if exists "Leaderboard is visible for open rooms" on public.leaderboard;
create policy "Leaderboard is visible for open rooms" on public.leaderboard for select
to anon using (
  exists (select 1 from public.rooms r where r.id = room_id and r.status in ('waiting', 'live', 'ended'))
);

drop policy if exists "Admins can read own leaderboard" on public.leaderboard;
create policy "Admins can read own leaderboard" on public.leaderboard for select
to authenticated using (
  exists (select 1 from public.rooms r where r.id = room_id and r.admin_id = auth.uid())
);

-- 8. Storage bucket for question imports
...
create policy "Admins can upload question imports" on storage.objects for insert
...
create policy "Admins can read own question imports" on storage.objects for select
to authenticated using (
  bucket_id = 'question-imports' and owner = auth.uid()
);
...
begin
  ...
  ) then alter publication supabase_realtime add table public.room_participants; end if;

  if not exists (
    select 1 from pg_publication_tables
    where pubname = 'supabase_realtime' and schemaname = 'public' and tablename = 'student_answers'
  ) then alter publication supabase_realtime add table public.student_answers; end if;

  if not exists (
    select 1 from pg_publication_tables
    where pubname = 'supabase_realtime' and schemaname = 'public' and tablename = 'leaderboard'
  ) then alter publication supabase_realtime add table public.leaderboard; end if;
end $$;

-- ============================================================
-- 10. CREATE ADMIN USER (adminmath / adminhebat)
-- This creates the Supabase Auth user for the admin account
-- ============================================================

-- Create the auth user for admin (if not exists)
do $$
declare
  v_user_id uuid;
  v_existing_id uuid;
begin
  ...
  select id into v_existing_id from auth.users where email = 'adminmath@mathbattle.app' limit 1;
  ...
  if v_existing_id is null then
    ...
    insert into auth.users (
      ...
      'adminmath@mathbattle.app',
      crypt('adminhebat', gen_salt('bf')),
      now(),
      'authenticated',
      'authenticated',
      now(),
      ...
      '{"username":"adminmath","full_name":"Admin MathBattle"}'::jsonb,
      false,
      '',
      '',
      '',
    ...
    values (v_user_id, 'Admin MathBattle', 'MathBattle Platform', 'admin')
    on conflict (id) do nothing;
    ...
    update auth.users
    set
      encrypted_password = crypt('adminhebat', gen_salt('bf')),
    ...
    insert into public.admins (id, full_name, school_name, role)
    values (v_existing_id, 'Admin MathBattle', 'MathBattle Platform', 'admin')
    on conflict (id) do nothing;

    raise notice 'Admin user updated with id: %', v_existing_id;
  end if;
end $$;

-- Done!
select 'Setup selesai! Tabel, RLS, dan admin user sudah siap.' as status;
